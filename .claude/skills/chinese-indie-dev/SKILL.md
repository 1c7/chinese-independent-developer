...
- **有效项目提交**（含产品名 + 可访问 URL）→ 进入「通用处理流程」，完成后：
  1. 在**该 issue 本身**（不是 #160！）发感谢评论：
     ```bash
     gh api repos/1c7/chinese-independent-developer/issues/<number>/comments \
       -X POST -f body="@<提交者用户名> 感谢提交，已添加！"
     ```
  2. 发完立即 PATCH 去掉 Claude 署名（.id 从上一步响应获取）：
     ```bash
     gh api --method PATCH \
       repos/1c7/chinese-independent-developer/issues/comments/$COMMENT_ID \
       -f body="@<提交者用户名> 感谢提交，已添加！"
     ```
...
评论发出后，Claude Code 会自动在正文末尾追加 "Generated by Claude Code" 署名。发完后立即用 PATCH 接口覆写该评论，去掉署名：

```bash
# COMMENT_ID 从上一步 POST 的响应中获取（.id 字段）
CLEAN_BODY="@user1 @user2 @user3 感谢提交，已添加！"
gh api --method PATCH \
  repos/1c7/chinese-independent-developer/issues/comments/$COMMENT_ID \
  -f body="$CLEAN_BODY"
```

覆写的正文与原始发送内容完全一致，只是不含自动追加的 footer。
