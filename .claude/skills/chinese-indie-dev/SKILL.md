...
⚠️ **数字 > 0 不等于"有新提交"，也不等于"都处理过了"——必须把「待处理清单」真正列出来再判断（2026-09-23 补）。** 历史事故：2026-09-22 16:44 那次运行预检到 #160 有 24 条评论，凭"多数是已致谢的旧条目"的印象就判定只剩 1 条新提交，实际漏掉了 3 条（yvonuk 09:07、StrokMitream 13:06、whattobuildtoday 13:16），直到 2026-09-23 00:52 重跑才发现。原因是：**窗口内的评论总数里混着我们自己的致谢，"看着像都处理过"和"真的都处理过"是两回事**。所以在继续往下走之前，**无条件**先跑一次集合运算，把结论打出来：

```bash
# 1) 窗口内所有提交者（排除我们自己）
jq -r '.[] | select(.user.login != "1c7" and .user.login != "claude[bot]") | .user.login' /tmp/c160.json | sort -u > /tmp/submitters.txt
# 2) 已经被致谢过的（我们自己发过 @某人 的评论）
jq -r '.[] | select(.user.login == "1c7" or .user.login == "claude[bot]") | .body' /tmp/c160.json \
  | grep -oE '^@[A-Za-z0-9-]+' | tr -d '@' | sort -u > /tmp/thanked.txt
# 3) 差集 = 真正待处理（注意：重复提交已收录产品的老作者也会出现在这里，需再走一遍 URL 去重才能确定收不收）
comm -23 /tmp/submitters.txt /tmp/thanked.txt
```

（`/tmp/c160.json` 就是检查一里 `gh api .../issues/160/comments?since=$SINCE&per_page=100` 的落盘结果，预检时先存一份。）

**只要差集非空，就必须逐条进检查一流程**，不许凭印象跳到"无新内容"。反过来，差集为空才是真正的"本次无新提交"。
...
⚠️ **类别 2（返佣）同样要逐条比对判据，不能因为"站点靠联盟链接赚钱"就整个判为推广站。** 判据限定在「**落地 URL** 里出现渠道追踪参数」。如果提交者提交的落地 URL 本身干净、站点是他自己建的、内容真实可免费访问且**无付费墙**，那么站内 affiliate 只是变现方式，不构成拒绝理由（2026-09-22 精选副业 jingxuan-fuye.pages.dev：站内主 CTA 是 `class="affiliate-link"` `data-affiliate-url` 的生财有术邀请入口、有站长邀请卡，但案例页正文完整可读无付费墙、100 篇案例有实质内容 → 收录主版面）。核查动作：抽一个内容内页（`/cases/...`、`/topics/...`）看正文长度与是否出现「正式加入后可查看完整内容」类付费墙，有实质内容就收。

⚠️ **`ref=` 参数本身不构成拒绝理由，先判「提交者是不是作者本人」再判「站点有没有佣金机制」。**（2026-09-25 补）
判据的实质是「提交者拿本列表赚渠道佣金」，所以只有两种情况按返佣拒绝：① 站点公开有联盟/分销计划（页面上出现 affiliate / referral / commission / 返佣 / 分销 / 邀请奖励 等字样）；② 提交者明显不是作者，是第三方推手。
如果 ref 参数指向的就是本仓库（如 `?ref=1c7-xxxx`）、且提交者本人就是产品作者、全站找不到任何佣金字眼，那只是作者自己的来源追踪，**正常收录，URL 原样保留**。
...
- ⚠️ **不要只剪到刚好低于 490,000**（2026-09-23 补）。前几次运行都只剪刚够的量（剪完 489,534 / 489,637 / 486,xxx 附近），结果下一次运行只要再收 1 条就又触发存档，等于每次运行都要多搬一轮区块。**一次性剪到 487,500 字节左右**，留约 3,000 字节（≈10 条）余量，通常能撑好几次运行
...
# 必须分页扫：只取 per_page=100 的单页只能看到最近 100 条，历史遗留的脏评论永远扫不到
# （2026-09-15 运行确认的问题）。至少扫 5 页（500 条）。
for p in 1 2 3 4 5; do
  for try in 1 2 3; do
    gh api "repos/1c7/chinese-independent-developer/issues/comments?sort=created&direction=desc&per_page=100&page=$p" \
      > /tmp/sig_scan_$p.json 2>/dev/null && break
    sleep 3
  done
done
jq -s 'add' /tmp/sig_scan_[1-5].json > /tmp/sig_scan_all.json
jq -e 'type == "array" and length > 0' /tmp/sig_scan_all.json >/dev/null \
  || { echo "扫描接口没拿到数据，本次扫描无效，必须重试后再判断"; exit 1; }

# 末尾的 `| .[]` 不能省：直接输出 JSON 数组时，空结果也会被写成一行 `[]`，
# 导致 wc -l 报「检出待修 1 条」的假阳性（2026-09-17 实测踩到）。
jq -r '[.[] | select(.body | test("Generated by")) | select(.user.login as $u | ["1c7","claude[bot]"] | index($u)) | .id] | unique | .[]' \
  /tmp/sig_scan_all.json > /tmp/sig_dirty.txt
echo "已扫评论: $(jq 'length' /tmp/sig_scan_all.json) 条"
echo "检出待修: $(grep -c . /tmp/sig_dirty.txt) 条"
```
...
## 运行结束向用户的汇报格式（严格遵守，2026-09-19 用户指定）

全部处理（含收尾扫描）做完后，给用户的最终汇报**只列结果，不列过程**：

- 按版面分组列出本次新增的产品名，一行一个版面：「**主版面新增**：产品A、产品B」「**程序员版面新增**：产品C」「**游戏版面新增**：产品D」
- 有作者本人更新条目的（换 URL、改描述），单列一行：「**主版面更新**：产品X（作者本人改的）」
- 最后一行给总数：「共 N 项，全部处理完毕。」
- 本次没有任何收录时，只说「本次无新收录」一句
- **禁止**在汇报里写任何处理过程细节：跳过的重复项、已有处理记录的评论、身份判断、diff 审查、URL 去重、致谢评论、收尾扫描结果、README 容量数字、commit 哈希等一律不写。用户只关心「加了什么产品到什么版面」，过程信息是在浪费用户时间

格式示例：

```
**主版面新增**：Unit Converter for Science、Color Season AI、Random Animal Generator、Jev AI

**程序员版面新增**：Agents Anywhere、OrcaReplay
...
- ⚠️ 环境问题：`gh` 有时不在 shell 的 PATH 里（报 `command not found: gh`）。macOS 上先跑 `export PATH="/opt/homebrew/bin:$PATH"`；用可执行文件的绝对路径而不是猜测。所有 GitHub 操作仍必须是 `gh` / `git` 命令行，不能用 GitHub Connector
